...
| Field | Value |
|-------|-------|
| wmap version in use | **Published, unlinked** — `package.json` pins `^0.19.0`, `node_modules/@paulrobins/wafermap` is a real npm install (not a symlink), `package-lock.json` resolves 0.19.0. The wmap↔tsmap boundary rework batch (issue #31) and the guide/help-system rework (issue #32) are both fully published as of this version — no outstanding linked-dev-only changes. |
| Latest wmap release | 0.19.0 — `openUserGuide(): void` exported on both `WaferMapController` and `GalleryController` (independent of `showHelpButton`), the guide window itself gained `window.open()`-with-`openFloatingWindow`-fallback (mirrors the gallery-detach pattern), plus a same-version fix for a detached-`<script>` timing bug that left every live-demo `data-wmap-demo` div empty (`populateGuideContent` split into `buildGuideContent`/`activateGuideScripts`, the latter now correctly called only after the content is connected to its document — see issue #32). Check [github.com/telecasterer/wafermap/releases](https://github.com/telecasterer/wafermap/releases) |
...
**Two smaller, independently-justified wmap-side gaps noted during the original review — still open, still non-blocking:**

1. **Zero test coverage on the 13 live demo widgets** (`docs/guide-demos.js`'s `populateGuideDemos`, one `if/else` branch per `data-wmap-demo` id) or on `openUserGuideWindow`'s DOM wiring. A lightweight smoke test — mount each demo id against a stub `__wmapDemoApi`, assert no throw — would catch a demo silently breaking without needing full visual verification. **Worth escalating now**, given the third-pass regression above shipped past whatever manual testing wmap did and was only caught by tsmap's own integration testing.
2. **`scripts/build-user-guide.mjs`'s image handling silently drops any markdown image it can't inline** (a regex strips references it doesn't recognize as an icon or the one hand-picked inlined screenshot, rather than warning or failing the build). A future contributor adding a new guide image would get no signal that it vanished from the shipped guide.

Neither is blocking; noting them here so they're not lost, not requesting they be actioned on this pass.
