...
| Latest wmap release | 0.17.0 (pending publish) — adds chrome + canvas theming via `--wmap-*` custom properties (whole toolbar/panel/menu/tooltip **and** the wafer canvas now follow `--wmap-*`; additive, light defaults baked in), plus a `jet` colour scheme and gallery auto-column packing. Note: issue #16 (public icon export) remains open. Check [github.com/telecasterer/wafermap/releases](https://github.com/telecasterer/wafermap/releases) |
...
### 25. wmap render chrome has no theming — hardcoded-light island in a dark host; needs a token system (light/dark/custom)
...
**tsmap status:** no workaround possible today — the embedded wmap render is simply light in tsmap's dark theme, and tsmap owns none of those colours. Blocked on wmap. When wmap ships the tokens, tsmap sets the `--wmap-*` variables from its own `--var` theme tokens on the render container (and re-applies on the `prefers-color-scheme` change tsmap already handles) — or, more simply, passes `theme: 'auto'` and lets wmap follow the OS. tsmap is a natural first consumer to validate the token set against a real dark theme.
