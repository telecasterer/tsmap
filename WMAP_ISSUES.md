...
| Last updated | 2026-07-03 (wmap **0.17.0** installed + adopted: issue #25 fixed — `--wmap-*` chrome + canvas theming tokens. tsmap unlinked from local `../wmap`, `package.json` pins `^0.17.0`, `node_modules` resolves 0.17.0. All 8 tsmap themes mapped to `--wmap-*` in `index.html`'s base `:root` (theme-independent aliasing via tsmap's own `--var` tokens — no per-theme duplication). `npm run check` clean, 168 JS tests green.) |
