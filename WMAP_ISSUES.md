...
| Field | Value |
|-------|-------|
| wmap version in use | **Currently linked** (`npm run wmap:link` — `node_modules/@paulrobins/wafermap` is a symlink to `../wmap`, not the npm install) for an ongoing batch of changes — see issue #31 below for the full list and rationale (the wmap↔tsmap boundary rework: moving reusable chart/analysis/report code from tsmap into wmap, a new wmap-owned "Analysis" tab, and native report sections). `package.json` still pins the published `^0.18.0` range, untouched. **Local uncommitted changes in `../wmap`** (none published yet, batch keeps growing): the full chart suite (`packages/charts/*` — capability, boxplot, histogram, correlation, scatter, bin pareto/cluster, the generic `barPanel.ts`), `canvas-adapter/analysisTab.ts` (shared Analysis tab, used by both `renderWaferGallery.ts` and `renderWaferMap.ts` via the opt-in `analysisEnabled` option), wafer-open-from-chart wiring (`onOpen` on `barPanel.ts`/`boxplot.ts`), `core/metadata.ts`'s first-class `WaferMetadata.split` field, `core/dies.ts`'s `isYieldEligibleDie`, `stats/facets.ts`/`capability.ts`/`correlation.ts`/`boxplot.ts`/`histogram.ts`/`scatter.ts`/`binPareto.ts`/`yield.ts` (the stats layer backing all of the above), and — most recently — `renderSummaryReport.ts`'s `renderLotSummaryReportHtml` gaining native Process Capability + Splits sections and internal multi-lot identity grouping (no more pre-computed `lotSummary` param), plus `summaryPanel.ts`'s pre-existing "Summary report" button updated to match. Remember to commit all of this in the wmap repo and roll it into the next `npm run wmap:unlink` + version bump before any tsmap release — do not unlink piecemeal mid-batch. |
